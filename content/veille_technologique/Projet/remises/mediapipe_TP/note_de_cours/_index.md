...
modularité : un ensemble d’outils prêt à l’emploi (mains, visage, corps, segmentation, etc.)
...
1. Palm Detection
Ce modèle est entraîné pour détecter uniquement la paume, car c’est la partie la plus stable visuellement. Détecter une paume est plus rapide et plus robuste que détecter une main entière.
2. Hand Landmark Model
Ce modèle prend une image cropée de la main et prédit les 21 landmarks avec une grande précision.
...
Extraction de la zone de la main
Passage dans le modèle Landmark
Production des 21 points
...
En réduisant la résolution à 640x480, MediaPipe peut fonctionner entre 8 et 12 FPS sur un Pi 4.
6. Limites techniques
6.1. Dépendance à la lumière
Une pièce trop sombre ou avec un contre-jour fort peut faire perdre le tracking.
6.2. Occlusions
Si des doigts sont cachés, le modèle perd facilement la structure.
6.3. Profondeur imprécise
Pas de vraie 3D mesurable.
6.4. Mouvements très rapides
Certains points peuvent “trembler”.
6.5. Pas conçu pour l’industrie
Ce n’est pas un système de vision professionnel haut de gamme comme Vicon ou Optitrack.
...
8.2. Interfaces main → ordinateur
contrôle du curseur
...
Un humain fait une démonstration
Le système enregistre la position des articulations
Le robot apprend à reproduire la tâche
